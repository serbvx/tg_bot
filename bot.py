...
def download_tiktok_via_tikwm(url: str, download_dir: str) -> list[str]:
...
def download_tiktok_via_tiklydown(url: str, download_dir: str) -> list[str]:
    """Скачивает через api.tiklydown.eu.org — запасной сервис, если tikwm недоступен."""
    resp = requests.get(
    ...
    if not video_url:
        raise RuntimeError("tiklydown API не вернул ссылку на видео")
...
def download_tiktok_via_api(url: str, download_dir: str) -> list[str]:
    """
    Пробует по очереди несколько сторонних сервисов для скачивания TikTok
    без водяного знака. Поднимает RuntimeError с деталями всех попыток,
    если ни один не сработал.
    """
    errors = []
    for name, fn in (
        ("tikwm", download_tiktok_via_tikwm),
        ("tiklydown", download_tiktok_via_tiklydown),
    ):
        try:
            return fn(url, download_dir)
